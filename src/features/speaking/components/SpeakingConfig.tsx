...
import {
  Mic, Volume2, Play, Trash2, Loader2,
  ChevronRight,
} from 'lucide-react'
import { useSpeakingStore, type SpeakingMode, type CEFRLevel, type SavedBatchSession } from '../stores/speakingStore'

const MODES: { value: SpeakingMode; label: string; desc: string; icon: typeof Mic }[] = [
  { value: 'pronunciation', label: 'Kiểm tra phát âm', desc: 'Đọc câu → AI chấm phát âm', icon: Mic },
  { value: 'shadowing', label: 'Nhại giọng', desc: 'Nghe → Lặp lại → AI chấm', icon: Volume2 },
]
...
const TOPICS = ['Daily Life', 'Travel', 'Food', 'Work', 'Health', 'Technology', 'Education', 'Shopping']

export function SpeakingConfig() {
  const {
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                    active ? 'gradient-bg' : 'bg-surface-800/50'
                  }`}>
                    <Icon className="w-4 h-4 text-white" />
                  </div>
                  <div>
                    <p className={`text-xs font-semibold ${active ? 'text-primary-300' : 'text-surface-100'}`}>
                      {m.label}
                    </p>
      ...
      >
        {loading ? (
          ...
          <>
            ...
            Bắt đầu luyện tập ({batchSize} bài)
          </>
        )}
      </button>
