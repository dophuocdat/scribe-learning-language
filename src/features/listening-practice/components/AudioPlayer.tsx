...
/**
 * AudioPlayer for listening exercises.
 *
 * Uses the same `useTTS` hook as VocabularyCard (which works on mobile).
 * Key: speak() must be called SYNCHRONOUSLY from user click — no async, no setTimeout.
 *
 * For long text: the TTS proxy has 200-char limit, but `useTTS.speak()` already handles
 * the fallback chain (proxy → speechSynthesis). On mobile, long text will be truncated
 * by the proxy but still produces audio for the first ~200 chars. The user can replay.
 */
export function AudioPlayer({ text }: AudioPlayerProps) {
  ...
  // Called DIRECTLY from onClick — synchronous, preserves user gesture on mobile
  const handlePlay = () => {
    if (isSpeaking) {
      stop()
      return
    ...
    // Exact same pattern as VocabularyCard: speak() called synchronously from click
  ...
  return (
    <div className="glass-card p-4 relative z-20">
      <div className="flex items-center gap-3">
        {/* Play/Pause — MUST be user-initiated click */}
        <button
          onClick={handlePlay}
