import { CheckCircle2, XCircle, AlertTriangle, RotateCcw, ArrowRight, Star } from 'lucide-react'
import { useListeningPracticeStore, type DictationContent } from '../stores/listeningPracticeStore'

export function DictationResult() {
  const { dictationResult, content, batchItems, currentBatchIndex, reset, advanceToNext, generateExercise } =
    useListeningPracticeStore()
  if (!dictationResult) return null

  const dictContent = content as DictationContent
  const { accuracy, total_words, correct_words, word_comparison, feedback_vi, score, xp_earned } = dictationResult
  ...
  const handleNext = () => {
    if (!advanceToNext()) {
      // No more in batch — generate new batch
      generateExercise()
    }
  }
